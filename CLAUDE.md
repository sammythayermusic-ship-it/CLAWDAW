...
5. Then the actual work for the session. Phase 2 first-10 is done, undo is solid, streaming events work. Reasonable next bites: (a) Phase 3 UI scaffolding (Tauri shell, track list, transport bar, plugin chain view) — the engine has enough contract surface to wire this up, and the streaming events make state sync cheap; (b) MIDI / clip RPCs (`ImportAudioFile`, `AddMidiNote`, `MoveRegion`, etc.) if you want to do something other than mix in the engine; (c) refactor `engine/src/main.cc` (now ~1070 lines, well past comfortable for one file) into per-area files (`broadcaster.h/cc`, `track_handlers.cc`, `plugin_handlers.cc`, etc.). Pick based on what you want to demo next.
