...
5. Then the actual work for the session. Phase 2 first-10 is done. Reasonable next bites: (a) the side-band undo bridge for parameter mutations — make `SetTrackVolume`/`SetTrackPan`/`SetPluginParameter` actually undoable by tracking before/after pairs in our own `commit_id` map; (b) `SubscribeEvents` even just for `RescanPlugins` progress would massively improve UX and unblock all our streaming RPCs; (c) Phase 3 UI scaffolding (Tauri shell, track list, transport bar) — engine has enough contract surface to wire this up. Pick based on what you want to demo next. Don't refactor `engine/src/main.cc` unless it's actually in the way; it's at ~600 lines and starting to feel cramped, but per-area splits can wait.
