...
5. Then the actual work for the session. Phase 2 first-10 is done and the undo path is now solid. Reasonable next bites: (a) `SubscribeEvents` — even a v1 with just `RescanPlugins` progress events would massively improve UX and unblock all our streaming RPCs; (b) Phase 3 UI scaffolding (Tauri shell, track list, transport bar) — the engine has enough contract surface to wire this up; (c) MIDI / clip RPCs (`ImportAudioFile`, `AddMidiNote`, etc.) if we want to do something other than mix in the engine. Pick based on what you want to demo next. Don't refactor `engine/src/main.cc` unless it's actually in the way; it's at ~650 lines and starting to feel cramped, but per-area splits can wait.
